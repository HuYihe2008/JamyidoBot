...
async def video_detail(url, **kwargs):
    try:
        async with aiohttp.request("GET", url) as resp:
            ...
            vurl = f"https://www.bilibili.com/video/{res['bvid']}"
...
async def live_detail(url):
    try:
        ...
        logger.exception("直播间解析出错--Error:\n")
        return "", ""
